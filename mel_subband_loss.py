...
class MelSubbandLoss(nn.Module):
	...
	def forward(self, enhance, clean):
		...
		if weight_sum.item() <= 0.0:
			...
			loss_spec = torch.sum(weights_t * mse_tensor) / weight_sum
		...
		s_target = (torch.sum(enh * cleanz, dim=-1, keepdim=True) * cleanz) / (torch.sum(cleanz * cleanz, dim=-1, keepdim=True) + self.eps)
		e_noise = enh - s_target
		# 能量比（dB）
		nr = torch.sum(s_target * s_target, dim=-1) + self.eps
		dr = torch.sum(e_noise * e_noise, dim=-1) + self.eps
		sisnr = -10.0 * torch.log10(nr / dr)
		sisnr_loss = sisnr.mean()

		return loss_spec + sisnr_loss

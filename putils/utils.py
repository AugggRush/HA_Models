...
def unit_norm_init(num_freq_bins: int) -> torch.Tensor:
    """创建用于单位范数归一化的初始化数组。
    
    Args:
        num_freq_bins (int): 频率bins的数量
        
    Returns:
        torch.Tensor: 形状为 [1, num_freq_bins] 的初始化数组
    """
    # 和Rust版本中的UNIT_NORM_INIT常量对应
    start, end = -9.0, 3.0  
    
    # 使用numpy创建等间距数组
    arr = np.linspace(start, end, num_freq_bins)
    
    # 转换为torch tensor并reshape为[1, num_freq_bins]
    arr = torch.from_numpy(arr).float()
    arr = arr.view(1, num_freq_bins)

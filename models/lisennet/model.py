...
class PlModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict({'train_g_loss': g_loss.to(self.device), 
                       'train_d_loss': d_loss.to(self.device)}, 
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if pesq_score is not None:
            self.log_dict({'val_g_loss': g_loss.to(self.device), 
                           'val_d_loss': d_loss.to(self.device), 
                           'pesq_score': pesq_score.to(self.device)}, 
    ...
    def pesq_loss(clean, noisy, sr=16000):
        ...
        noisy = librosa.resample(noisy, orig_sr=24000, target_sr=sr)
        clean = librosa.resample(clean, orig_sr=24000, target_sr=sr)        
        try:

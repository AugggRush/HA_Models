...
class Encoder(nn.Module):
    def __init__(self, in_channels, num_channels=16):
        ...
        self.conv_1 = nn.Sequential(
            ...
            CustomLayerNorm((1, 129), stat_dims=(1, 3)),
            nn.PReLU(num_channels // 4),
        )
        
        self.conv_2 = DSConv(num_channels//4, num_channels//2, n_freqs=129)
        self.conv_3 = DSConv(num_channels//2, num_channels//4*3, n_freqs=64)
        self.conv_4 = DSConv(num_channels//4*3, num_channels, n_freqs=32)
...
class MaskDecoder(nn.Module):
    def __init__(self, num_features, num_channels=64, out_channel=2, beta=1):
        ...
        self.up1 = USConv(num_channels * 2, num_channels // 4 * 3, n_freqs=16)
        self.up2 = USConv(num_channels // 4 * 3 * 2, num_channels // 2, n_freqs=32)  # 128
        self.up3 = USConv(num_channels // 2 * 2, num_channels // 4, n_freqs=64)  # 256
...
class LiSenNet(nn.Module):
    def __init__(self, num_channels=8, n_blocks=1, n_fft=256, hop_length=48, compress_factor=0.3):
        ...
        self.stft = STFT_asym(filter_length=n_fft, hop_length=hop_length, 
                              win_length=n_fft, window='asqrthann', M=hop_length)
    ...
    def apply_stft(self, x, return_complex=True):
        ...
        if return_complex:
            return torch.view_as_complex(spec.squeeze(1))
        else:
    ...
    def apply_istft(self, x, length=None):
        ...
        x = x.unsqueeze(1)
        m_output = self.stft.inverse_cpx(x)
        m_output = m_output.squeeze(-1)
        m_output = torch.nn.functional.pad(m_output, (0, length-m_output.shape[1]))        
        # audio = torch.istft(
        #     x,
        #     self.n_fft,
        #     self.hop_length,

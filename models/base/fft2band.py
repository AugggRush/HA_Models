...
class BandConverter(nn.Module):
    ...
    def forward_band_mat(self, stft_mag, method="magnitude"):
        ...
        if method == "energy":
            stft_mag = stft_mag ** 2
        # print("stft_mag: ", stft_mag.shas.to_band_matrix.shape)
    ...
    def get_segment_index(self):
        ...
        if self.band_method == "bark":
            ...
            while True:
                ...
                if len(np.unique(band_seg_idx)) == out_band_num:  # remove repeat band index
                    band_seg_idx = np.unique(band_seg_idx).astype(int)
                    print(f"real band_num which used for generating band segmentation: {self.band_num} ")
                    break
                self.band_num += 1
                assert self.band_num < self.freq_bins, "invalid band_num"
            ...
            band_seg_idx = -(EarQ * minBW) + np.exp((np.arange(1, self.band_num + 1)) * (-np.log(highFreq + EarQ * minBW) + \
                    np.log(lowFreq + EarQ * minBW)) / self.band_num) * (highFreq + EarQ * minBW)
...
if __name__ == '__main__':
    ...
    ipt = torch.rand([4, 100, 257])
    print(f"================================band trans================================")
    converter = BandConverter(band_num=256, freq_bins=257, fs=16000, band_method="erb")
    opt1 = converter.forward_band_mat(ipt)
    opt2 = converter.forward_band_mat(ipt)
    # print(f"opt1 {opt1}")
    # print(f"opt2 {opt2}")
    print(f"trans abs error {torch.abs(opt1 - opt2).mean()}")

    print(f"===============================inv band trans================================")
    ipt = torch.randn(16, 10, 64, device=device)

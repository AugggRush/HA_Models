...
from putils.utils import as_complex, as_real, get_device, unit_norm_init
    ...
    def forward(self, spec: Tensor, mask: Tensor, atten_lim: Optional[Tensor] = None) -> Tensor:
        ...
        if atten_lim is not None:
            ...
            if self.clamp_tensor:
                ...
                mask = mask.clamp(min=atten_lim.view(-1, 1, 1, 1).contiguous())
            else:
                m_out = []
...
class MultiFrameModule(nn.Module):
    ...
    def spec_unfold_real(self, spec: Tensor):
        if self.need_unfold:
            ...
            return spec.permute(0, 1, 5, 2, 3, 4).contiguous()
...
class DF(MultiFrameModule): 
    ...
    def forward(self, spec: Tensor, coefs: Tensor):
        ...
        coefs = coefs.contiguous()
        spec_u = self.spec_unfold(torch.view_as_complex(spec))
        coefs = torch.view_as_complex(coefs)
        spec_f = spec_u.narrow(-2, 0, self.num_freqs)
        coefs = coefs.view(coefs.shape[0], -1, self.frame_size, coefs.shape[2], coefs.shape[3]).contiguous()
        if self.conj:
            coefs = coefs.conj()
        spec_f = df(spec_f, coefs)
        
        spec_out = spec.clone()
        spec_out[..., : self.num_freqs, :] = torch.view_as_real(spec_f)
        return spec_out
...
class ExponentialUnitNorm(nn.Module):
    ...
    def __init__(self, alpha: float, num_freq_bins: int, eps: float = 1e-14):
        ...
        s = torch.from_numpy(unit_norm_init(num_freq_bins)).view(1, 1, num_freq_bins, 1).contiguous()
...
class DfOp(nn.Module):
    ...
    def forward_real_unfold(
        ...
        padded = padded.permute(0, 1, 4, 2, 3).contiguous()
...
def assign_df(spec: Tensor, spec_f: Tensor, df_bins: int, alpha: Optional[Tensor]):
    ...
    if alpha is not None:
        b = spec.shape[0]
        alpha = alpha.view(b, 1, -1, 1, 1).contiguous()

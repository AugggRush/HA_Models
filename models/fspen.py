...
class SUBBAND_ENCODE(nn.Module):
    def __init__(self, in_ch=1,out_ch=[8, 8, 8, 8,8], kernel_size=[2, 3, 6, 10, 20], stride_size=[1, 1,2,5,10]) -> None:
        super().__init__()
        # 8 7 6 7 6 || 16 21 30 70 120 
        self.conv1d_1 = nn.Conv1d(in_ch, out_ch[0], kernel_size[0], stride_size[0],padding=0)
        self.conv1d_2 = nn.Conv1d(in_ch, out_ch[1], kernel_size[1], stride_size[1], padding=0)
        self.conv1d_3 = nn.Conv1d(in_ch, out_ch[2], kernel_size[2], stride_size[2], padding=1)
        self.conv1d_4 = nn.Conv1d(in_ch, out_ch[3], kernel_size[3], stride_size[3], padding=3)
        self.conv1d_5 = nn.Conv1d(in_ch, out_ch[4], kernel_size[4], stride_size[4], padding=5)
    ...
    def forward(self, x):
        ...
        # x = x.permute(0,2,3,1)
        # x = x.reshape(b*t,c,f)
        x1 = self.act(self.norm1d_1(self.conv1d_1(x[...,:4])))
        x2 = self.act(self.norm1d_2(self.conv1d_2(x[...,4:10])))
        x3 = self.act(self.norm1d_3(self.conv1d_3(x[...,10:17])))
        x4 = self.act(self.norm1d_4(self.conv1d_4(x[...,17:35])))
        x5 = self.act(self.norm1d_5(self.conv1d_5(x[...,35:])))

        return x1,x2,x3,x4,x5


class SUBBAND_DECODE(nn.Module):
    def __init__(self, in_ch=[8, 8, 8, 8, 8],out_ch=[1, 1, 2, 5, 10]) -> None:
        super().__init__()
...
class DPCRN_Light(nn.Module):
    def __init__(self, n_fft=128, hop_len=48, win_len=128,
                 in_ch=2, out_ch=2, filter_size=[2,8], kernel_size=[6,8],stride_size=[2,2], numUnits=8):
        ...
        # self.conv1X1d = nn.Conv1d(filter_size[1], filter_size[1], 1,1)
        ...
        self.deconv1 = nn.Sequential(nn.Conv1d(filter_size[1]*2, filter_size[1], 1, 1),
        ...
        self.deconv3 = nn.Sequential(nn.Conv1d(filter_size[0]*2, filter_size[0], 1, 1),
                                     nn.ConvTranspose1d(filter_size[0], out_ch, kernel_size[0], stride_size[0], padding=2, output_padding=1))
        self.denorm1d_1 = nn.BatchNorm1d(filter_size[0])
        self.denorm1d_2 = nn.BatchNorm1d(out_ch)
        # self.denorm1d_3 = nn.BatchNorm1d(out_ch)
        self.dprnn = DPRNN_GRU(numUnits, 16, 16)
        self.dprnn1 = DPRNN_GRU(numUnits, 16, 16)
        # self.dprnn2 = DPRNN_GRU(numUnits, 8, 16)
    ...
    def forward(self, x):
        ...
        feat = torch.stack([spec_real, spec_imag], dim=1)  # (B,2,T,F)
        
        # bftc 
        x_ref = feat.permute(0,3,2,1)
        ...
        x_rnn_in = x2.reshape(b,t,self.filter_size[-1], -1)
        ...
        rnn_out = rnn_out.reshape(b*t, self.filter_size[-1], -1) # b*t c f
        # rnn_out = self.deconv1_fm(rnn_out)
        o_1 = torch.cat([rnn_out, x2], 1)
        o_1 = self.deconv1(o_1)
        o_1 = self.denorm1d_1(o_1)
        o_1 = self.act(o_1)
        o_2 = torch.cat([o_1, x1], 1)
        o_2 = self.act(self.denorm1d_2(self.deconv3(o_2)))
        # o_3 = torch.cat([o_2, x1], 1)
        # o_3 = self.deconv3(o_3)
        out = o_2.reshape(b,t,c,-1)
        ...
        spec_enh = crm_out.permute(0,2,1,3).unsqueeze(1) # B, C, T, F, 2
...
class FSPEN(DPCRN_Light):
    def __init__(self, in_ch=2, out_ch=2, filter_size=[1,4,8], kernel_size=[6,8,6],stride_size=[2,2,2], numUnits=4) -> None:
